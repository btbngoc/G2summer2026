...
(() => {
  ...
  function setSlot(id, val) {
    if (!id) return;
    if (loaded) sessionEdited.add(id);
  ...
  async function toDataUrl(file, w) {
    const bitmap = await createImageBitmap(file);
    try {
      const big = Math.max(bitmap.width, bitmap.height);
      if (big <= MAX_DIM && file.size <= PASSTHROUGH_MAX_BYTES) {
        try { return await readAsDataUrl(file); } catch { /* fall through */ }
      }
      const scale = Math.min(1, MAX_DIM / big);
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      const man = (this.id && window.IMAGE_SLOT_MANIFEST &&
        window.IMAGE_SLOT_MANIFEST[this.id]) || null;
      const edited = this.id && sessionEdited.has(this.id);
      if (man && !edited) stored = null;
      const srcAttr = (man && man.src) || this.getAttribute('src') || '';
      this._userUrl = (stored && stored.u) || null;
      const url = this._userUrl || srcAttr;
      const viewSrc = stored || man;
      
      if (!this.hasAttribute('data-reframe')) {
        this._view = {
          s: viewSrc && Number.isFinite(viewSrc.s) ? clampS(viewSrc.s) : 1,
          x: viewSrc && Number.isFinite(viewSrc.x) ? viewSrc.x : 0,
          y: viewSrc && Number.isFinite(viewSrc.y) ? viewSrc.y : 0,
        };
      }
